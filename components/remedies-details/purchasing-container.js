		...
		<div className="remedy-details-purchasing container _80">
			...
			<div className="product-details">
				<div className="product-details__title heading-3 mb-md">
					{details.subtitle || `Your ${title} Terpenes`}
				</div>
				...
				<div className="order-controls product-details__order-controls mb-lg">
								...
								onClick={async () => {
									...
									if (!context.checkout) {
										...
										if (mutation && mutation.data) {
											updateContext(
												'checkout',
												mutation.data?.checkoutCreate?.checkout?.id
											);
											url = mutation.data.checkoutCreate.checkout.webUrl;
										}
									} else {
										// otherwise, add a line item to the existing checkout
										mutation = await graphql(CART_ADD_LINE_ITEMS_MUTATION, {
											checkoutId: context.checkout,
											lineItems: [product],
										});
					...
					<div className="increment-controls product-details__increment-controls">
						<a
							...
							className="increment-controls__control increment-controls__control--increase"
						>
							+
						</a>
						<a
							onClick={() => quantity > 1 && setQuantity(quantity - 1)}
							className="increment-controls__control increment-controls__control--decrease"
						>
							-
						</a>
					</div>
